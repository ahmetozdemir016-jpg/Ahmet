...
' BANKA HESAP HAREKETLERİ -> EMANET / MİF ANALİZ RAPORU v8 - LibreOffice 7.2 Linux uyumlu
...
Sub BankaHareketleriEmanetRaporu()
    ...
    For r = headerRow + 1 To lastRow
        ...
        If desc <> "" Then
            ...
            WriteOneRowV2 oOut, outRow, oSrc, r, colTarih, colFis, colTutar, colBakiye, yon, kimlikTipi, kimlik, gonderen, borclu, durum, hbNo, eRef, extraRef, talimatNo, tur, banka, guven, mif, notu, desc
            toplam = toplam + 1
    ...
    MsgBox "Emanet_Rapor ve Özet_Rapor hazırlandı." & Chr(10) & "Giriş: " & girisAdet & " adet" & Chr(10) & "Çıkış/Aktarım: " & cikisAdet & " adet", 64, "Emanet Rapor v8"
End Sub

Sub WriteOneRowV2(oOut As Object, outRow As Long, oSrc As Object, srcRow As Long, colTarih As Long, colFis As Long, colTutar As Long, colBakiye As Long, yon As String, kimlikTipi As String, kimlik As String, gonderen As String, borclu As String, durum As String, hbNo As String, eRef As String, extraRef As String, talimatNo As String, tur As String, banka As String, guven As String, mif As String, notu As String, desc As String)
    Dim tcNo As String, vergiNo As String
    tcNo = ""
    vergiNo = ""
    If kimlikTipi = "TCKN" Then
        tcNo = kimlik
    ElseIf kimlikTipi = "VKN" Then
        vergiNo = kimlik
    End If

    PutText oOut, 0, outRow, CellText(oSrc, colTarih, srcRow)
    PutText oOut, 1, outRow, CellText(oSrc, colFis, srcRow)
    PutAmount oOut, 2, outRow, oSrc, colTutar, srcRow
    PutAmount oOut, 3, outRow, oSrc, colBakiye, srcRow
    PutText oOut, 4, outRow, yon
    PutText oOut, 5, outRow, tcNo
    PutText oOut, 6, outRow, vergiNo
    PutText oOut, 7, outRow, gonderen
    PutText oOut, 8, outRow, borclu
    PutText oOut, 9, outRow, durum
    PutText oOut, 10, outRow, hbNo
    PutText oOut, 11, outRow, eRef
    PutText oOut, 12, outRow, extraRef
    PutText oOut, 13, outRow, talimatNo
    PutText oOut, 14, outRow, tur
    PutText oOut, 15, outRow, banka
    PutText oOut, 16, outRow, guven
    PutText oOut, 17, outRow, mif
    PutText oOut, 18, outRow, notu
    PutText oOut, 19, outRow, desc
End Sub

Sub WriteReportHeaderV2(oOut As Object)
    oOut.getCellRangeByName("A1:T1").merge(True)
    oOut.getCellByPosition(0, 0).String = "Banka Hesap Hareketleri - Emanete Alma Analiz Raporu v6"
    oOut.getCellRangeByName("A2:T2").merge(True)
    oOut.getCellByPosition(0, 1).String = "Gönderen/ödeyen ile mükellef/borçlu ayrımı açıklama kalıplarına göre yapılmıştır. T.C. No ve Vergi No ayrı sütunlara yazılır; Kimlik No sütunu yoktur. Özet raporda yalnızca giriş/çıkış adet ve toplamları gösterilir."
    WriteSimpleHeaderV2 oOut, 3
End Sub

Sub WriteSimpleHeaderV2(oOut As Object, rowNo As Long)
    Dim h As Variant, i As Integer
    h = Array("Tarih", "Fiş No", "İşlem Tutarı", "Bakiye", "İşlem Yönü", "T.C. No", "Vergi No", _
              "Gönderen/Ödeyen", "Mükellef/Borçlu", "Gönderen-Borçlu Durumu", "Haciz Bildiri No (FRW)", _
              "E-Yazı/Tahakkuk Ref.", "Ek Yazı/Sıra No", "Talimat No", "Belge/Tür", "Banka/Kanal", "Ayırma Güveni", _
              "MİF Açıklama Önerisi", "Kontrol Notu", "Ham Açıklama")
    For i = 0 To UBound(h)
...
Function ExtractBorcluV2(desc As String, kimlik As String, gonderen As String, hbNo As String) As String
    ...
    If InStr(u, "TALIMAT NO:") > 0 Then
        ...
        If tmp <> "" Then ExtractBorcluV2 = tmp Else ExtractBorcluV2 = gonderen
        Exit Function
    End If
...
Function CompanyPartBeforePersonTC(s As String) As String
    ...
    pTC = FindTCPos(s)
    If pTC = 0 Then CompanyPartBeforePersonTC = "": Exit Function
    beforeTC = Left(s, pTC - 1)
    p = LastCompanyMarkerEnd(beforeTC)
    If p > 0 Then CompanyPartBeforePersonTC = CleanSpaces(Left(beforeTC, p)) Else CompanyPartBeforePersonTC = ""
End Function

Function PersonPartBeforeTC(s As String) As String
    Dim pTC As Long, beforeTC As String, p As Long
    pTC = FindTCPos(s)
    If pTC = 0 Then PersonPartBeforeTC = "": Exit Function
    beforeTC = Left(s, pTC - 1)
    p = LastCompanyMarkerEnd(beforeTC)
    If p > 0 Then PersonPartBeforeTC = CleanSpaces(Mid(beforeTC, p + 1)) Else PersonPartBeforeTC = ""
End Function

Function LastCompanyMarkerEnd(s As String) As Long
    Dim u As String, p As Long, best As Long, marker As Variant, arr As Variant
    u = Norm(s)
    arr = Array("LTD. ŞTİ", "LTD.ŞTİ", "LIMITED SIRKETI", "LİMİTED ŞİRKETİ", "ANONIM SIRKETI", "ANONİM ŞİRKETİ", "A.Ş.", "A.S.")
    best = 0
    For Each marker In arr
        p = InStrRev(u, Norm(CStr(marker)))
        If p > 0 Then If p + Len(CStr(marker)) - 1 > best Then best = p + Len(CStr(marker)) - 1
    Next marker
...
Function AfterGond(desc As String) As String
    ...
    If p > 0 Then AfterGond = Trim(Mid(desc, p + 5)) Else AfterGond = Trim(desc)
End Function
...
Sub WriteSummaryV6(oSum As Object, girisAdet As Long, girisTop As Double, cikisAdet As Long, cikisTop As Double)
    ...
    h = Array(Array("Kriter", "Adet", "Toplam Tutar"), _
              Array("Giriş", girisAdet, girisTop), _
              Array("Çıkış/Aktarım", cikisAdet, cikisTop))
    For i = 0 To UBound(h)
        oSum.getCellByPosition(0, i).String = h(i)(0)
        oSum.getCellByPosition(1, i).Value = h(i)(1)
        oSum.getCellByPosition(2, i).Value = h(i)(2)
    Next i
...
Sub FormatReportV2(oOut As Object, lastOutRow As Long)
    ...
    widths = Array(2600, 2300, 3000, 3000, 3000, 3600, 3600, 6800, 7600, 4200, 5600, 5600, 3300, 3300, 5200, 6500, 3600, 9500, 9000, 13000)
    For i = 0 To 19: oOut.Columns(i).Width = widths(i): Next i
    oOut.Rows(0).Height = 650: oOut.Rows(1).Height = 900: oOut.Rows(3).Height = 850
    oOut.getCellRangeByName("A1:T1").CharWeight = 150
    oOut.getCellRangeByName("A1:T1").CharHeight = 14
    oOut.getCellRangeByName("A1:T1").HoriJustify = 2
    oOut.getCellRangeByName("A1:T1").CellBackColor = RGB(31, 78, 121)
    oOut.getCellRangeByName("A1:T1").CharColor = RGB(255, 255, 255)
    oOut.getCellRangeByName("A2:T2").IsTextWrapped = True
    oOut.getCellRangeByName("A4:T4").CharWeight = 150
    oOut.getCellRangeByName("A4:T4").CellBackColor = RGB(31, 78, 121)
    oOut.getCellRangeByName("A4:T4").CharColor = RGB(255, 255, 255)
    oOut.getCellRangeByName("A4:T4").IsTextWrapped = True
    If lastOutRow >= 4 Then
        oOut.getCellRangeByPosition(0, 4, 19, lastOutRow).IsTextWrapped = True
        oOut.getCellRangeByPosition(2, 4, 3, lastOutRow).NumberFormat = GetNumberFormat(ThisComponent, "#,##0.00")
        oOut.getCellRangeByPosition(5, 4, 13, lastOutRow).NumberFormat = GetNumberFormat(ThisComponent, "@")
    End If
    ThisComponent.CurrentController.freezeAtPosition(0, 4)
End Sub

Sub FormatControlV2(oOut As Object, lastOutRow As Long)
    FormatReportV2 oOut, lastOutRow
    oOut.getCellRangeByName("A1:T1").CellBackColor = RGB(198, 89, 17)
End Sub
...
Function RemoveEntityStopWords(u As String) As String
    Dim arr As Variant, w As Variant
    arr = Array(" SANAYI ", " SAN ", " TICARET ", " TIC ", " LIMITED ", " LTD ", " SIRKETI ", " STI ", " ANONIM ", " AS ", " VE ")
    u = " " & u & " "
    For Each w In arr
        u = Replace(u, CStr(w), " ")
    Next w
...
Function CellText(oSheet As Object, col As Long, row As Long) As String
    If col < 0 Then CellText = "" Else CellText = oSheet.getCellByPosition(col, row).String
End Function
...
Function ExtractFRW(desc As String) As String
    ...
    Do While e <= Len(desc)
        ...
        e = e + 1
    Loop
    If s < p And e > p + 3 Then ExtractFRW = UCase(Mid(desc, s, e - s)) Else ExtractFRW = ""
End Function
...
Function ExtractERef(desc As String) As String
    ...
    For i = 1 To Len(rest)
        ch = Mid(rest, i, 1)
        If (ch >= "0" And ch <= "9") Or ch = "E" Or ch = "e" Or ch = "-" Or ch = "." Then out = out & ch Else Exit For
    Next i
...
Function CutBeforeKeywords(s As String, keys As Variant) As String
    ...
    Next i
    If best > 1 Then CutBeforeKeywords = Left(s, best - 1) Else CutBeforeKeywords = s
End Function
...
Function FindFirstPositive(arr As Variant) As Long
    ...
    best = 0
    For i = 0 To UBound(arr)
        If arr(i) > 0 Then If best = 0 Or arr(i) < best Then best = arr(i)
    Next i
...
Function GetNumberFormat(oDoc As Object, fmt As String) As Long
    Dim oFormats As Object, locale As New com.sun.star.lang.Locale
    oFormats = oDoc.NumberFormats
    GetNumberFormat = oFormats.queryKey(fmt, locale, True)
    If GetNumberFormat = -1 Then GetNumberFormat = oFormats.addNew(fmt, locale)
End Function
