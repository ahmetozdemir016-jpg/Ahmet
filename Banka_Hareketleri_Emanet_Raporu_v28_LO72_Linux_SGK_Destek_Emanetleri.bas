...
' BANKA HESAP HAREKETLERİ -> EMANET / MİF ANALİZ RAPORU v28 - LibreOffice 7.2 Linux uyumlu
' Bu sürümde sadece Emanet_Rapor ve Özet_Rapor oluşturulur; Kimlik No, Ayırma Güveni ve Kontrol Notu sütunları yoktur.
' Belge No: Sadece Giriş kayıtlarına verilir. Çıkış/Aktarım kayıtlarında boş bırakılır.
' Emanet Türü: Çıkış/Aktarım kayıtlarında boş bırakılır.
' İşlem Tutarı ve Bakiye: Türkçe sayı formatı #.##0,00 olarak yazılır.
' Emanet Türü: SGK-KOSGEB2026 için 129-2026; SGK-STB2026 için 130-2026; teminat/plaka için 44; DFIF/D.F.I.F için 33; diğerleri için 39.
' Plaka No: Açıklamada plaka/plakalı/araç teminatı kalıpları varsa ayrı sütuna alınır.
' Giriş kayıtlarında Fiş No'ya bakmadan tarih + sıra numarası ile oluşturulur.
' Örnek: 05.06.2026 tarihli ilk Giriş kaydı => 20260605000000000001, ikinci Giriş kaydı => 20260605000000000002.
...
Sub BankaHareketleriEmanetRaporu()
    ...
    Dim emanetTuru As String, plakaNo As String
    Dim toplam As Long, farkli As Long, belirsiz As Long, orta As Long, dusuk As Long, kimliksiz As Long
    Dim girisAdet As Long, cikisAdet As Long, belgeSeq As Long
    ...
    For r = headerRow + 1 To lastRow
        ...
        If desc <> "" Then
            ...
            emanetTuru = GuessEmanetTuru(desc)
            plakaNo = ExtractPlakaNo(desc)
            ...
            If plakaNo <> "" Then
                mif = Trim(mif & " Plaka No: " & plakaNo)
            End If

            WriteOneRowV2 oOut, outRow, oSrc, r, colTarih, colFis, colTutar, colBakiye, yon, belgeSeq, kimlikTipi, kimlik, gonderen, borclu, durum, hbNo, eRef, extraRef, talimatNo, tur, banka, mif, desc, emanetTuru, plakaNo
            toplam = toplam + 1
    ...
    MsgBox "Emanet_Rapor ve Özet_Rapor hazırlandı." & Chr(10) & "Giriş: " & girisAdet & " adet" & Chr(10) & "Çıkış/Aktarım: " & cikisAdet & " adet", 64, "Emanet Rapor v28"
End Sub

Sub WriteOneRowV2(oOut As Object, outRow As Long, oSrc As Object, srcRow As Long, colTarih As Long, colFis As Long, colTutar As Long, colBakiye As Long, yon As String, belgeSeq As Long, kimlikTipi As String, kimlik As String, gonderen As String, borclu As String, durum As String, hbNo As String, eRef As String, extraRef As String, talimatNo As String, tur As String, banka As String, mif As String, desc As String, emanetTuru As String, plakaNo As String)
    ...
    If yon = "Çıkış/Aktarım" Then
        PutText oOut, 2, outRow, ""
    Else
        PutText oOut, 2, outRow, BuildSiraliBelgeNo(tarihText, belgeSeq)
    End If
    PutAmount oOut, 3, outRow, oSrc, colTutar, srcRow
    PutAmount oOut, 4, outRow, oSrc, colBakiye, srcRow
    PutText oOut, 5, outRow, yon
    PutText oOut, 6, outRow, emanetTuru
    PutText oOut, 7, outRow, plakaNo
    PutText oOut, 8, outRow, tcNo
    PutText oOut, 9, outRow, vergiNo
    PutText oOut, 10, outRow, gonderen
    PutText oOut, 11, outRow, borclu
    PutText oOut, 12, outRow, durum
    PutText oOut, 13, outRow, hbNo
    PutText oOut, 14, outRow, eRef
    PutText oOut, 15, outRow, extraRef
    PutText oOut, 16, outRow, talimatNo
    PutText oOut, 17, outRow, tur
    PutText oOut, 18, outRow, banka
    PutText oOut, 19, outRow, mif
    PutText oOut, 20, outRow, desc
End Sub
...
Sub WriteSimpleHeaderV2(oOut As Object, rowNo As Long)
    ...
    h = Array("Tarih", "Fiş No", "Belge No", "İşlem Tutarı", "Bakiye", "İşlem Yönü", "Emanet Türü", "Plaka No", _
              "T.C. No", "Vergi No", "Gönderen/Ödeyen", "Mükellef/Borçlu", "Gönderen-Borçlu Durumu", _
              "Haciz Bildiri No (FRW)", "E-Yazı/Tahakkuk Ref.", "Ek Yazı/Sıra No", "Talimat No", "Belge/Tür", _
              "Banka/Kanal", "MİF Açıklama Önerisi", "Ham Açıklama")
    For i = 0 To UBound(h)
...
Sub WriteSummaryV6(oSum As Object, girisAdet As Long, girisTop As Double, cikisAdet As Long, cikisTop As Double)
    ...
    On Error Resume Next
    oSum.getCellRangeByName("C2:C3").NumberFormat = GetAmountNumberFormat(ThisComponent)
    On Error GoTo 0
End Sub

Sub FormatReportV2(oOut As Object, lastOutRow As Long)
    Dim i As Integer, widths As Variant
    widths = Array(2600, 2300, 4200, 3000, 3000, 3000, 6500, 3600, 3600, 3600, 6800, 7600, 4200, 5600, 5600, 3300, 3300, 5200, 6500, 9500, 13000)
    For i = 0 To 20
        oOut.Columns(i).Width = widths(i)
    Next i
    oOut.Rows(0).Height = 850
    oOut.getCellRangeByName("A1:U1").CharWeight = 150
    oOut.getCellRangeByName("A1:U1").CellBackColor = RGB(31, 78, 121)
    oOut.getCellRangeByName("A1:U1").CharColor = RGB(255, 255, 255)
    oOut.getCellRangeByName("A1:U1").IsTextWrapped = True
    If lastOutRow >= 1 Then
        oOut.getCellRangeByPosition(0, 1, 20, lastOutRow).IsTextWrapped = True
        On Error Resume Next
        oOut.getCellRangeByPosition(3, 1, 4, lastOutRow).NumberFormat = GetAmountNumberFormat(ThisComponent)
        oOut.getCellRangeByPosition(2, 1, 2, lastOutRow).NumberFormat = GetNumberFormat(ThisComponent, "@")
        oOut.getCellRangeByPosition(5, 1, 20, lastOutRow).NumberFormat = GetNumberFormat(ThisComponent, "@")
        On Error GoTo 0
    End If
...
Sub FormatControlV2(oOut As Object, lastOutRow As Long)
    ...
    oOut.getCellRangeByName("A1:U1").CellBackColor = RGB(198, 89, 17)
End Sub
...
Sub PutAmount(oOut As Object, outCol As Long, outRow As Long, oSrc As Object, srcCol As Long, srcRow As Long)
    ' İşlem Tutarı / Bakiye hücrelerini gerçek sayı olarak yazar.
    ' Böylece D ve E sütunlarında #.##0,00 formatı düzgün görünür.
    Dim txt As String, ok As Boolean, v As Double
    ...
    Else
        txt = Trim(oSrc.getCellByPosition(srcCol, srcRow).String)
        v = ParseAmountText(txt, ok)
        If ok = True Then
            oOut.getCellByPosition(outCol, outRow).Value = v
        Else
            oOut.getCellByPosition(outCol, outRow).String = txt
        End If
    End If
End Sub

Function ParseAmountText(s As String, ByRef ok As Boolean) As Double
    ' Türkçe/İngilizce yazılmış tutar metinlerini Double değere çevirir.
    ' Örnekler: 1.234,56 | 1234,56 | 1,234.56 | -4310163,88
    Dim t As String, i As Long, ch As String, decSep As String
    Dim lastComma As Long, lastDot As Long, digitPos As Long
    Dim result As Double, decDiv As Double, neg As Boolean, hasDigit As Boolean, decSeen As Boolean
    ...
    For i = 1 To Len(t)
        ch = Mid(t, i, 1)
        digitPos = InStr("0123456789", ch)
        If digitPos > 0 Then
            hasDigit = True
            If decSeen = True Then
                result = result + ((digitPos - 1) / decDiv)
                decDiv = decDiv * 10
            Else
    ...
    If neg = True Then result = 0 - result
    ok = True
    ParseAmountText = result
End Function



Function BuildSiraliBelgeNo(tarihText As String, outRow As Long) As String
    ' Belge No artık fiş numarasından üretilmez.
    ' Sadece Giriş kayıtları için sıra numarası kullanılır; Çıkış/Aktarım satırlarında boş bırakılır.
    ' Örnek: 05.06.2026 + 1 => 20260605000000000001
    Dim ymd As String, seqText As String
    ymd = DateTextToYYYYMMDD(tarihText)
    If ymd = "" Then
        BuildSiraliBelgeNo = ""
        Exit Function
    End If
    seqText = PadLeftNumber(CStr(outRow), 12)
    BuildSiraliBelgeNo = ymd & seqText
End Function
...
Function BuildUniqueBelgeNo(tarihText As String, fisText As String, oOut As Object, outRow As Long) As String
    ' Önce normal Belge No oluşturulur. Daha önce aynı Belge No yazılmışsa
    ' son rakam bölümü 1 artırılarak boş/benzersiz numara bulunur.
    Dim baseNo As String
    baseNo = BuildBelgeNo(tarihText, fisText)
    If baseNo = "" Then
        BuildUniqueBelgeNo = ""
        Exit Function
    End If
    If IsBelgeNoUsed(oOut, baseNo, outRow) = False Then
        BuildUniqueBelgeNo = baseNo
        Exit Function
    End If
    BuildUniqueBelgeNo = NextUnusedBelgeNo(oOut, baseNo, outRow)
End Function

Function IsBelgeNoUsed(oOut As Object, belgeNo As String, outRow As Long) As Boolean
    Dim i As Long, existing As String
    IsBelgeNoUsed = False
    If belgeNo = "" Then Exit Function
    For i = 1 To outRow - 1
        existing = Trim(oOut.getCellByPosition(2, i).String)
        If existing = belgeNo Then
            IsBelgeNoUsed = True
            Exit Function
        End If
    Next i
...
Function NextUnusedBelgeNo(oOut As Object, baseNo As String, outRow As Long) As String
    Dim prefix As String, tailText As String, tailVal As Double, width As Long, candidate As String
    Dim codePart As String, isCoded As Boolean, guard As Long
    If Len(baseNo) < 20 Then
        NextUnusedBelgeNo = baseNo & "-2"
        Exit Function
    End If

    codePart = Mid(baseNo, 11, 3)
    isCoded = IsAllLetters(codePart)

    If isCoded Then
        prefix = Left(baseNo, 13)
        tailText = Right(baseNo, 7)
        width = 7
    Else
        prefix = Left(baseNo, 8)
        tailText = Right(baseNo, 12)
        width = 12
    End If

    tailVal = ToLongDigits(tailText)
    guard = 0
    Do
        tailVal = tailVal + 1
        candidate = prefix & PadLeftNumber(DoubleToPlainDigits(tailVal), width)
        guard = guard + 1
        If IsBelgeNoUsed(oOut, candidate, outRow) = False Then
            NextUnusedBelgeNo = candidate
            Exit Function
        End If
        If guard > 10000 Then Exit Do
    Loop
    NextUnusedBelgeNo = baseNo
End Function

Function DoubleToPlainDigits(n As Double) As String
    ...
    p = InStr(s, ".")
    If p > 0 Then s = Left(s, p - 1)
    DoubleToPlainDigits = OnlyDigits(s)
End Function
...
Function PadLeftNumber(s As String, width As Long) As String
    s = Trim(s)
...
Function GuessEmanetTuru(desc As String) As String
    ' EVDB emanet türü için otomatik öneri.
    ' SGK-KOSGEB2026 açıklamalarında: 129-2026 SGK KOSGEB Destek Ödeme Emanetleri
    ' SGK-STB2026 açıklamalarında: 130-2026 SGK STB Destek Ödeme Emanetleri
    ' D.F.I.F / DFIF benzeri işlerde: 33 Kişilere Ait Diğer Emanetler
    ' Araç/plaka/ihale/teminat içerikli işlerde: 44 Vergi Teminatları
    ' Diğerlerinde: 39 Diğer Çeşitli Emanetler
    Dim u As String, plakaBulundu As String
    u = Norm(desc)
    plakaBulundu = ExtractPlakaNo(desc)

    ' Genel yazıdaki ayırıcı ifade açıklamanın içinde geçen KOSGEB2026 / STB2026 bilgisidir.
    ' Banka açıklaması örnekleri:
    ' SGK-KOSGEB2026;BORCNO:...;VKN/TCKN:...;UNVAN/AD-SOYAD;VD ADI;KOSGEB2026-DESTEK
    ' SGK-STB2026;BORCNO:...;VKN/TCKN:...;UNVAN/AD-SOYAD;VD ADI;STB2026-DESTEK
    If InStr(u, "SGK-KOSGEB2026") > 0 Or InStr(u, "KOSGEB2026") > 0 Or InStr(u, "KOSGEB 2026") > 0 Then
        GuessEmanetTuru = "129-2026 SGK KOSGEB Destek Ödeme Emanetleri"
        Exit Function
    End If

    If InStr(u, "SGK-STB2026") > 0 Or InStr(u, "STB2026") > 0 Or InStr(u, "STB 2026") > 0 Then
        GuessEmanetTuru = "130-2026 SGK STB Destek Ödeme Emanetleri"
        Exit Function
    End If

    If InStr(u, "D.F.I.F") > 0 Or InStr(u, "D.FIF") > 0 Or InStr(u, "DFIF") > 0 Or InStr(u, "D F I F") > 0 Then
        GuessEmanetTuru = "33 Kişilere Ait Diğer Emanetler"
        Exit Function
    End If

    ' Ekranda görülen örnekler:
    ' 16 JME 71 IHALE KATILIM TEMINATI
    ' 16APJ590 PLAKALI ARAC ALIM BEDELI / ARAC TEMINATI BEDELI
    ' Bu tip kayıtlar mutlaka 44 Vergi Teminatları olmalı.
    If InStr(u, "TEMINAT") > 0 Then
        GuessEmanetTuru = "44 Vergi Teminatları"
        Exit Function
    End If

    If InStr(u, "IHALE") > 0 And InStr(u, "KATILIM") > 0 Then
        GuessEmanetTuru = "44 Vergi Teminatları"
        Exit Function
    End If

    If InStr(u, "PLAKALI") > 0 Or InStr(u, "PLAKA") > 0 Then
        GuessEmanetTuru = "44 Vergi Teminatları"
        Exit Function
    End If

    If plakaBulundu <> "" Then
        If InStr(u, "ARAC") > 0 Or InStr(u, "IHALE") > 0 Or InStr(u, "ALIM BEDELI") > 0 Then
            GuessEmanetTuru = "44 Vergi Teminatları"
            Exit Function
        End If
    End If

    GuessEmanetTuru = "39 Diğer Çeşitli Emanetler"
End Function

Function ExtractPlakaNo(desc As String) As String
    ' Açıklama içinden plaka numarasını bulur.
    ' Örnek: "16ABC123 PLAKALI" => "16 ABC 123"
    ' Örnek: "Plaka No: 34 AB 1234" => "34 AB 1234"
    Dim u As String, p As Long, part As String, res As String
    u = Norm(desc)

    p = InStr(u, "PLAKALI")
    If p > 0 Then
        part = Left(desc, p - 1)
        res = FindLastPlateInText(part)
        If res <> "" Then
            ExtractPlakaNo = res
            Exit Function
        End If
    ...
    p = InStr(u, "PLAKA NO")
    If p > 0 Then
        part = Mid(desc, p + Len("PLAKA NO"), 45)
        res = FindFirstPlateInText(part)
        If res <> "" Then
            ExtractPlakaNo = res
            Exit Function
        End If
    ...
    If InStr(u, "TEMINAT") > 0 Or InStr(u, "ARAC") > 0 Then
        res = FindFirstPlateInText(desc)
        If res <> "" Then
            ExtractPlakaNo = res
            Exit Function
        End If
...
Function FindFirstPlateInText(s As String) As String
    Dim tokens As Variant, i As Long, cand As String
    tokens = Split(PlateCleanText(s), " ")
    If UBound(tokens) < 0 Then
        FindFirstPlateInText = ""
        Exit Function
    End If

    For i = 0 To UBound(tokens)
        cand = PlateFromCompactToken(CStr(tokens(i)))
        If cand <> "" Then
            FindFirstPlateInText = cand
            Exit Function
        End If
        If i <= UBound(tokens) - 1 Then
            cand = PlateFromTwoTokens(CStr(tokens(i)), CStr(tokens(i + 1)))
            If cand <> "" Then
                FindFirstPlateInText = cand
                Exit Function
            End If
        End If
        If i <= UBound(tokens) - 2 Then
            cand = PlateFromThreeTokens(CStr(tokens(i)), CStr(tokens(i + 1)), CStr(tokens(i + 2)))
            If cand <> "" Then
                FindFirstPlateInText = cand
                Exit Function
            End If
...
Function FindLastPlateInText(s As String) As String
    Dim tokens As Variant, i As Long, cand As String, lastCand As String
    tokens = Split(PlateCleanText(s), " ")
    lastCand = ""
    If UBound(tokens) < 0 Then
        FindLastPlateInText = ""
    ...
    For i = 0 To UBound(tokens)
        ...
        End If
    Next i
    FindLastPlateInText = lastCand
End Function

Function PlateCleanText(s As String) As String
    Dim u As String
    u = Norm(s)
    u = Replace(u, ":", " ")
    u = Replace(u, ";", " ")
    ...
    Do While InStr(u, "  ") > 0
        u = Replace(u, "  ", " ")
    Loop
    PlateCleanText = Trim(u)
End Function


Function PlateFromTwoTokens(a As String, b As String) As String
    ' 16AIF 410 veya 16 AIF410 gibi iki parçaya bölünmüş plaka yazımlarını yakalar.
    Dim city As String, letters As String, nums As String
    a = Trim(a)
    b = Trim(b)
    ...
    If Len(a) >= 3 Then
        city = Left(a, 2)
        letters = Mid(a, 3)
        If IsDigitsLen(city, 2, 2) = True And IsValidCityCode(city) = True And IsLettersLen(letters, 1, 3) = True And IsDigitsLen(b, 2, 4) = True Then
            PlateFromTwoTokens = city & " " & letters & " " & b
            Exit Function
        End If
    End If

    ' 16 AIF410
    If IsDigitsLen(a, 2, 2) = True And IsValidCityCode(a) = True Then
        If SplitLettersDigits(b, letters, nums) = True Then
            If IsLettersLen(letters, 1, 3) = True And IsDigitsLen(nums, 2, 4) = True Then
                PlateFromTwoTokens = a & " " & letters & " " & nums
                Exit Function
            End If
        End If
    End If
...
Function SplitLettersDigits(s As String, ByRef letters As String, ByRef nums As String) As Boolean
    Dim i As Long, ch As String
    letters = ""
    nums = ""
    s = Trim(s)
    If s = "" Then
        SplitLettersDigits = False
        Exit Function
    End If
...
Function PlateFromThreeTokens(a As String, b As String, c As String) As String
    a = Trim(a)
    b = Trim(b)
    c = Trim(c)
    If IsDigitsLen(a, 2, 2) = False Then
        PlateFromThreeTokens = ""
        Exit Function
    End If
    If IsValidCityCode(a) = False Then
        PlateFromThreeTokens = ""
        Exit Function
    End If
    If IsLettersLen(b, 1, 3) = False Then
        PlateFromThreeTokens = ""
        Exit Function
    End If
    If IsDigitsLen(c, 2, 4) = False Then
        PlateFromThreeTokens = ""
        Exit Function
    End If
    PlateFromThreeTokens = a & " " & b & " " & c
End Function

Function PlateFromCompactToken(t As String) As String
    Dim city As String, rest As String, letters As String, nums As String
    Dim i As Long, ch As String
    t = Trim(t)
    If Len(t) < 5 Or Len(t) > 9 Then
        PlateFromCompactToken = ""
        Exit Function
    End If
    city = Left(t, 2)
    If IsDigitsLen(city, 2, 2) = False Then
        PlateFromCompactToken = ""
        Exit Function
    End If
    If IsValidCityCode(city) = False Then
        PlateFromCompactToken = ""
        Exit Function
    End If
    ...
    If IsLettersLen(letters, 1, 3) = False Then
        PlateFromCompactToken = ""
        Exit Function
    End If
    If IsDigitsLen(nums, 2, 4) = False Then
        PlateFromCompactToken = ""
        Exit Function
    End If

    PlateFromCompactToken = city & " " & letters & " " & nums
End Function

Function ToLongDigits(s As String) As Double
    Dim i As Long, ch As String, n As Double, d As Long
    n = 0
    For i = 1 To Len(s)
        ch = Mid(s, i, 1)
        d = InStr("0123456789", ch)
        If d > 0 Then
            n = (n * 10) + (d - 1)
        End If
    Next i
...
Function IsDigitsLen(s As String, minLen As Long, maxLen As Long) As Boolean
    Dim i As Long, ch As String
    s = Trim(s)
    If Len(s) < minLen Or Len(s) > maxLen Then
        IsDigitsLen = False
        Exit Function
    End If
    For i = 1 To Len(s)
...
Function IsLettersLen(s As String, minLen As Long, maxLen As Long) As Boolean
    Dim i As Long, ch As String
    s = Trim(s)
    If Len(s) < minLen Or Len(s) > maxLen Then
        IsLettersLen = False
...
Function IsValidCityCode(s As String) As Boolean
    Dim n As Double
    s = OnlyDigits(s)
    If s = "" Then
...
Function GetAmountNumberFormat(oDoc As Object) As Long
    ' Resimdeki sayı biçimi: #.##0,00  (örnek: 1.234,57)
    ' LibreOffice 7.2 Linux uyumu için addNew kullanılmaz.
    ' Önce Türkçe biçim kodu aranır, yoksa mevcut standart sayı biçimine düşülür.
    Dim oFormats As Object, locale As New com.sun.star.lang.Locale, k As Long
    locale.Language = "tr"
    locale.Country = "TR"
    oFormats = oDoc.NumberFormats

    k = oFormats.queryKey("#.##0,00", locale, True)
    If k <> -1 Then
        GetAmountNumberFormat = k
        Exit Function
    End If
    ...
    On Error Resume Next
    k = oFormats.getStandardFormat(com.sun.star.util.NumberFormat.NUMBER, locale)
    If Err <> 0 Then
        Err = 0
        k = 0
    End If
...
Function GetNumberFormat(oDoc As Object, fmt As String) As Long
    ' Genel biçim sorgusu. Yeni format eklemez; varsa kullanır.
    Dim oFormats As Object, locale As New com.sun.star.lang.Locale, k As Long
    locale.Language = "tr"
    locale.Country = "TR"
    oFormats = oDoc.NumberFormats
