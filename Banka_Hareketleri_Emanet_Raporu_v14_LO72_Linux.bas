...
' BANKA HESAP HAREKETLERİ -> EMANET / MİF ANALİZ RAPORU v14 - LibreOffice 7.2 Linux uyumlu
' Bu sürümde sadece Emanet_Rapor ve Özet_Rapor oluşturulur; Kimlik No, Ayırma Güveni ve Kontrol Notu sütunları yoktur.
' Tarih + Fiş No kullanılarak Belge No üretir: 04.06.2026 + F15161 => 20260604000000015161
...
Function BuildBelgeNo(tarihText As String, fisText As String) As String
    ' Belge No formatı: YYYYMMDD + Fiş No içindeki rakamların 12 haneye tamamlanmış hali.
    ' Örnek: 04.06.2026 + F15161 => 20260604000000015161
    Dim ymd As String, digits As String
    ymd = DateTextToYYYYMMDD(tarihText)
    digits = OnlyDigits(fisText)
    If ymd = "" Or digits = "" Then
        BuildBelgeNo = ""
        Exit Function
    End If
    Do While Len(digits) < 12
        digits = "0" & digits
    Loop
    If Len(digits) > 12 Then
        digits = Right(digits, 12)
    End If
    BuildBelgeNo = ymd & digits
End Function
