...
' BANKA HESAP HAREKETLERİ -> EMANET / MİF ANALİZ RAPORU v12 - LibreOffice 7.2 Linux uyumlu
...
Sub BankaHareketleriEmanetRaporu()
    ...
    MsgBox "Emanet_Rapor ve Özet_Rapor hazırlandı." & Chr(10) & "Giriş: " & girisAdet & " adet" & Chr(10) & "Çıkış/Aktarım: " & cikisAdet & " adet", 64, "Emanet Rapor v12"
End Sub
...
Sub WriteOneRowV2(oOut As Object, outRow As Long, oSrc As Object, srcRow As Long, colTarih As Long, colFis As Long, colTutar As Long, colBakiye As Long, yon As String, kimlikTipi As String, kimlik As String, gonderen As String, borclu As String, durum As String, hbNo As String, eRef As String, extraRef As String, talimatNo As String, tur As String, banka As String, mif As String, desc As String)
    ...
    PutText oOut, 16, outRow, mif
    PutText oOut, 17, outRow, desc
End Sub
...
Sub FormatReportV2(oOut As Object, lastOutRow As Long)
    ...
    widths = Array(2600, 2300, 3000, 3000, 3000, 3600, 3600, 6800, 7600, 4200, 5600, 5600, 3300, 3300, 5200, 6500, 9500, 13000)
    For i = 0 To 17
        oOut.Columns(i).Width = widths(i)
    Next i
    oOut.Rows(0).Height = 850
    oOut.getCellRangeByName("A1:R1").CharWeight = 150
    oOut.getCellRangeByName("A1:R1").CellBackColor = RGB(31, 78, 121)
    oOut.getCellRangeByName("A1:R1").CharColor = RGB(255, 255, 255)
    oOut.getCellRangeByName("A1:R1").IsTextWrapped = True
    If lastOutRow >= 1 Then
        oOut.getCellRangeByPosition(0, 1, 17, lastOutRow).IsTextWrapped = True
        On Error Resume Next
        oOut.getCellRangeByPosition(2, 1, 3, lastOutRow).NumberFormat = GetNumberFormat(ThisComponent, "#,##0.00")
        oOut.getCellRangeByPosition(5, 1, 13, lastOutRow).NumberFormat = GetNumberFormat(ThisComponent, "@")
        On Error GoTo 0
    End If
...
Sub FormatControlV2(oOut As Object, lastOutRow As Long)
    ...
    oOut.getCellRangeByName("A1:R1").CellBackColor = RGB(198, 89, 17)
End Sub
